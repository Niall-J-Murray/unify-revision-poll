...
resource "aws_eip" "nat" {
  ...
  tags = merge(var.tags, {
    Name = "${var.subdomain_name}-nat-eip"
  })
}
...
resource "aws_nat_gateway" "nat" {
  ...
  subnet_id     = aws_subnet.public_a.id # Place NAT in the first public subnet

  tags = merge(var.tags, {
    Name = "${var.subdomain_name}-nat-gateway"
  })
...
# Private Route Table (routes outbound traffic via NAT Gateway in AZ A)
resource "aws_route_table" "private" {
  ...
  tags = merge(var.tags, {
    Name = "${var.subdomain_name}-private-rtb"
  })
}
...
resource "aws_route_table_association" "private_b" {
  ...
  route_table_id = aws_route_table.private.id # Associate with the same private route table
}
